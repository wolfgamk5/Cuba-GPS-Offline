...
class GraphHopperRoutingEngine(context: Context) {
    ...
    fun load(): Boolean {
        ...
        return try {
            ...
            // Throwable (no solo Exception): errores como NoSuchMethodError son subclases de
            // Error, no de Exception, y si solo se captura Exception se cuelan y cierran la app.
    ...
    /**
     * GraphHopper 10.x usa por defecto `DAType.RAM_STORE`, cuya clase `RAMDataAccess` invoca en
     * su inicializador estático `VarHandle.withInvokeExactBehavior()`. Ese método no existe en la
     * implementación de VarHandle del runtime ART de Android (aunque el dispositivo sea reciente),
     * así que la primera vez que esa clase se carga la app revienta con:
     * `NoSuchMethodError: No virtual method withInvokeExactBehavior()...`
     *
     * `GraphHopper` no expone un setter público para elegir el tipo de almacenamiento por defecto
     * (solo `setStoreOnFlush`, que alterna entre RAM y RAM_STORE, las dos clases problemáticas) —
     * la única otra vía es `init(GraphHopperConfig)`, pensada para el flujo de importación completo.
     * Por eso aquí se fuerza el campo privado `dataAccessDefaultType` a `DAType.MMAP` por reflexión:
     * así nunca se llega a cargar `RAMDataAccess` y en su lugar se usa `MMAPDataAccess`, totalmente
     * compatible con el grafo ya generado (el formato en disco no depende de este tipo de acceso).
     */
    private fun forceMmapDataAccess(gh: GraphHopper) {
        try {
            val field = GraphHopper::class.java.getDeclaredField("dataAccessDefaultType")
            field.isAccessible = true
            field.set(gh, DAType.MMAP)
        } catch (e: Exception) {
            Log.w(TAG, "No se pudo forzar DAType.MMAP en GraphHopper, se usará el valor por defecto", e)
        }
    }
